...
阶段十一 **11.3「对比 DPO vs RLHF 性能」** 的落地：一个 CPU 可验证的基准 harness，
在**同一个合成偏好任务**上分别跑既有 **DPO** 与 **PPO (RLHF)** 两种对齐方式，
报告可比的指标并给出观测结论。基准代码在 `llm/training/rlhf/aligner_benchmark.py`。
...
- `compare_dpo_vs_ppo(config, ...)`：一起跑两者并返回 `{dpo, ppo, summary}`。
...
result = compare_dpo_vs_ppo(config, dpo_epochs=40, ppo_steps=3, prompts=["hello", "world"])
...
"对比"：在同一合成任务上，DPO 是这里 CPU 可验证的更稳、更快的选择，而 PPO 的收敛
需要更大 rollout 预算 / 更稠密的奖励。

## 测试

`tests/training/test_aligner_benchmark.py` 覆盖：`TargetTokenReward` 的打分与 mask；
`preference_fraction` 的排序与形状校验；DPO 基准 e2e（偏好比例 0 -> ~1，强断言）；
PPO 基准（真实 `PPOTrainer` 产出有限 reward 轨迹）；以及 `compare_dpo_vs_ppo` 返回
summary 的端到端入口。
