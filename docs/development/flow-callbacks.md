...
- **`Callback.on_train_start(logs=None)`**
    - **触发点**: `TrainingEngine.run()` 的开头.
    - **参数**: `logs: dict | None` - 训练状态字典.
...
- **`Callback.on_epoch_start(epoch, logs=None)`**
    - **触发点**: `TrainingEngine.run()` 内部的 `for epoch in ...` 循环的开始处.
    - **参数**: `epoch: int`, `logs: dict | None` - 训练状态字典.
...
- **`Callback.on_batch_start(epoch, batch_idx, logs=None)`**
    - **触发点**: `TrainingEngine._run_epoch()` 内部的 `for batch_idx, batch in ...` 循环的开始处.
    - **参数**: `epoch: int`, `batch_idx: int`, `logs: dict | None`.
...
- **`Callback.on_train_step_end(epoch, batch_idx, loss, metrics, logs=None)`**
    - **触发点**: `TrainingEngine._run_epoch()` 中, 在 `scaler.step(optimizer)` 和 `scaler.update()` 之后.
    - **参数**: `epoch: int`, `batch_idx: int`, `loss: Tensor`, `metrics: dict`, `logs: dict | None`.
...
- **`Callback.on_batch_end(epoch, batch_idx, logs=None)`**
...
- **`Callback.on_validation_start(epoch, logs=None)`**
    - **触发点**: `TrainingEngine._run_validation_epoch()` 的开头.
    - **参数**: `epoch: int`, `logs: dict | None`.
    - **用途**: 准备验证环境.
...
- **`Callback.on_validation_end(epoch, logs=None)`**
    - **触发点**: `TrainingEngine._run_validation_epoch()` 的末尾.
    - **参数**: `epoch: int`, `logs: dict | None` - 包含验证结果, 例如 `{'val_loss': 0.123}`.
...
- **`Callback.on_save_checkpoint(epoch, logs=None)`**
...
- **`Callback.on_epoch_end(epoch, logs=None)`**
    ...
    - **参数**: `epoch: int`, `logs: dict | None` - 包含该 `epoch` 的聚合指标, 例如 `{'avg_loss': ..., 'val_loss': ...}`.
...
- **`Callback.on_exception(exception, logs=None)`**
    - **触发点**: `TrainingEngine.run()` 的 `except` 块中.
    - **参数**: `exception: Exception`, `logs: dict | None`.
    - **用途**: 执行自定义的异常处理逻辑.
