...
2. **注册你的任务**: 在 `training/tasks/builtin.py` 中通过 `TASK_REGISTRY.register()`
   注册你的新任务（并可选地在 `pyproject.toml` 的 `llm.tasks` entry-point 组注册）。
3. **运行训练**: 在命令行中执行以下命令:

    ```bash
    uv run llm-train --task <your_task_name>
    ```
