...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"  Getting first item (idx=0) from dataset:")
        item0 = dataset[0]
        ...
        expected_last_chunk_raw_len = len(all_tokens) % (max_len - overlap_val) if len(all_tokens) > max_len else len(all_tokens)
        # This calculation is a bit off for the general case.
        # The last chunk starts at some `i`. Its content is all_tokens[i:].
        # Then it's padded to max_len.
        num_non_pad = 0
        for token_id in item_last['input_ids']:
        ...
        print(f"  DataLoader created. Iterating through one batch...")
        ...
        for batch in dataloader:
            ...
            assert batch['input_ids'].shape == (2, max_len) or batch['input_ids'].shape[1] == max_len # Handles last partial batch
