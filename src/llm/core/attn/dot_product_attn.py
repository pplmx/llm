...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        参数:
            ...
            p (float): 注意力内部和最终输出的 Dropout 概率。
            is_causal (bool): 是否默认对所有调用应用因果掩码。
            bias (bool): 线性层是否使用偏置。
        """
        super().__init__()
    ...
    def forward(
        ...
        参数:
            ...
            attn_mask (Optional[Tensor]): 布尔注意力掩码。
                - 形状需能广播至 [B, N, S, S]。通常是 [B, S, S] 或 [B, 1, S, S]。
                - `True` 表示屏蔽。
            is_causal (Optional[bool]): 覆盖模块级别的 `is_causal` 设置。
                - 如果为 None, 使用 `self.is_causal`。
                - 如果为 True/False, 使用该值。
        ...
        # 决定是否使用因果掩码 (允许 forward 调用覆盖 __init__ 设置)
        ...
        query = query.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)  # [B, N, S, D]
        key = key.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)  # [B, N, S, D]
        value = value.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)  # [B, N, S, D]
        ...
        attn_output = scaled_dot_product_attention(
            ...
            key=key,
            value=value,
            attn_mask=attn_mask,  # 传递准备好的掩码
            # 只有在训练模式下才应用注意力内部的 dropout
            dropout_p=self.p if self.training else 0.0,
            is_causal=use_causal,  # 使用计算得出的因果标志
            scale=None,  # 使用默认 scale = 1/sqrt(head_dim)
        )  # 输出 [B, N, S, D]

        # 4. 重塑输出: [B, N, S, D] -> [B, S, N, D] -> [B, S, H]
