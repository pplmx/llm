...
class TestSimpleCharacterTokenizerInitialization:
    ...
    def test_corpus_containing_pad_char_string(self):
        ...
        unique_corpus_chars_inc_pad_str = sorted(list(set("".join(corpus_with_pad_str))))
        
        assert tokenizer.pad_char in unique_corpus_chars_inc_pad_str
        assert tokenizer.vocab_size == len(unique_corpus_chars_inc_pad_str) # No extra PAD added
        assert tokenizer.pad_token_id == tokenizer.stoi[tokenizer.pad_char]
        assert tokenizer.chars == unique_corpus_chars_inc_pad_str # PAD string is part of sorted unique chars
...
class TestSimpleCharacterTokenizerEncoding:
    ...
    def test_encode_extended(self, extended_tokenizer):
        text = "你好"
        expected_tokens = [
            extended_tokenizer.stoi.get('你'),
            extended_tokenizer.stoi.get('好')
        ]
        expected_tokens = [t for t in expected_tokens if t is not None]
