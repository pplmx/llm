...
- [x] **PEFT method registry** (T2 PEFT 切片完成: `PEFT_REGISTRY` + `apply_peft` / `merge_peft` / `unmerge_peft` / `disable_peft` / `enable_peft` / `get_peft_parameters` / `count_peft_parameters` 统一 dispatch; 7 个内置方法 (`lora` / `qlora` / `adalora` / `prefix_tuning` / `ia3` / `bitfit` / `adapter`) 注册为 `PEFTMethod` dataclass 记录; `llm.peft_methods` setuptools entry-point group 开放第三方扩展; `TrainingConfig.peft_method` + `peft_kwargs` 加性字段 (legacy `use_*` 标志路径保留); SFT/DPO 继承; Pydantic validator 在 config-load 时拒绝未知方法)
